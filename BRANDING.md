...
- The homepage has a navy editorial hero without the four-stage diagram, location strapline, or replay control. (`SystemsField` was deleted; it had been kept unused.) Hero images use `HeroBackdrop`; text and preview transitions remain restrained and respect reduced motion. Services uses in-flow illustrations and ordinary anchor targets.
- Shader decision: still no WebGL inside the system diagrams — HTML/SVG already expresses those relationships, and a shader would add GPU, context, and fallback duties without a clearer message. WebGL is used only as ambient light that carries no information: `AmbientShader` behind hero copy and the capabilities panel, and `PageAtmosphere` as a page-wide violet field at z-index -1. Every one of them is aria-hidden, declines to mount under prefers-reduced-motion or on low-end devices, and drops its context off-screen. The field's deepest colour is a contrast budget, not a style choice — see the note in `PageAtmosphere.tsx`. References: [MDN WebGL best practices](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API/WebGL_best_practices), [Motion SVG animation](https://motion.dev/docs/react-svg-animation).
