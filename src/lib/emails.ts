...
/**
 * The reply to someone who wrote in from the website. It repeats nothing
 * they typed, not even their name: anyone can put any address in the form,
 * so words echoed back would let a stranger send their own message from our
 * domain. The topic is shown only when it is one of the form's own choices.
 */
export function acknowledgementEmail(input: { topic?: string } = {}): string {
  const about = input.topic
    ? ` about <strong style="color:#1D1B22;">${escapeHtml(input.topic)}</strong>`
    : '';
